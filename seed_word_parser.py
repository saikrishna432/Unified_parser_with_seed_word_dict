...
def load_suffex_words(language):
    # load a separate prefix file... maybe use the set of seed words itself??
    prefix_words = sc.SortedList()
    path_suffiex_words=os.path.abspath("new_unified_parser/prefixes/suffixes_"+language+".txt")
    with open(path_suffiex_words, 'r') as fl:
        cnts = fl.readlines()
        for ln in cnts:
            wd = ln.strip()
            try:
...
def matching_based_parse2(word : str,lsflag : int, wfflag : int, clearflag : int, prefix_encodings, seed_word_encodings, language, debug=True):
    ...
    # print(suffix_encodings)
    match_prefix, pref_ans = [], []
    match_suffix, suf_ans = [], []
    for pref in prefixes:
        if prefix_encodings.count(''.join(pref[0])) != 0:
            # print(match_prefix)
            ...
            # break
    for suf in suffixes:
        if suffix_encodings.count(''.join(suf[0])) != 0:
            # print(match_prefix)
            match_suffix.append(suf)
    
    suf_start_index=len(word)-len(match_suffix[0])
    remword = spword[len(match_prefix[0]):suf_start_index]
    if g.flags.DEBUG:
        if len(match_prefix)==0:
            print(f'prefixes are not found')
        else:
            print(f'found prefix - {"".join(match_prefix)}')
        if len(match_suffix)==0:
            print(f'suffixes are not found')
        else:
            print(f'found suffix - {"".join(match_prefix)}')
...
def matching_based_parse(word : str,lsflag : int, wfflag : int, clearflag : int, prefix_encodings, seed_word_encodings, language, debug):
    ...
    total_matching_sublists=match_prefix + matching_sublists + match_suffix

    max_matching_sublists = weighted_activity_selection(word,total_matching_sublists, debug)
    if g.flags.DEBUG:
        print("max_matching_sublists:",max_matching_sublists)
    if g.flags.DEBUG:
        for l, _, _ in max_matching_sublists:
...
def generate_item(word : str, lsflag : int, wfflag : int, clearflag : int,language_main : str,seed_words_main):
    try:
        wd = word.strip()
        wd = RemoveUnwanted(wd)
        u_languages=["telugu","tamil","kannada","malayalam","odiya","panjabi","gujrathi"]
        wd_debug = True
        ...
        wd_debug=True
        parsing = matching_based_parse(wd,lsflag , wfflag, clearflag, prefix_words, seed_words, language, wd_debug)
        if g.flags.DEBUG:
...
if __name__ == '__main__':
    ...
    source_dir = os.path.dirname("seed_word_dict")
    seed_dict_file_name = os.path.basename("seed_word_dict")
    temp_path= seed_dict_file_name + "/"+'new_hind_seed_dict'
    seed_dict_path=os.path.join(source_dir, temp_path)
    with open(seed_dict_path, 'r') as fl:
        cnts = fl.readlines()
        for ln in cnts:
...
# now, we open the file containing all the hindi words
    ...
    with open(path_big_dict, 'r') as fl:
    ...
    for wd in tqdm(words):
        result=generate_item(wd,0,0,0,"hindi",seed_words_main)
        if result ==0:
            pass
        else:
            print(result)
    ...
    # with open(path_out_big_dict, 'a') as f:
