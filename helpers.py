...
def GetFile(g : GLOBALS, LangId : int, type : int) -> str:
    ...
    langIdNameMapping = { 1 : "malayalam", 2 : "tamil", 3 : "telugu",
        ...
        7 : "gujarathi", 8 : "odiya", 9 : "punjabi", 10 : "sanskrit" }
...
def ConvertToSymbols(g : GLOBALS, input : str) -> str:
    ...
    for j in range(len(str1)):
        #print(str1[j], j, ord(str1[j]), ord(str1[j])/128)
        if (ord(str1[j]) < 8204):
...
def Syllabilfy(phone : str,g) -> str:
    phonecopy = phone
    phonecopy = rec_replace(phonecopy, "&&","&")
    ...
    for i in range(0,45):
        ...
        c2 = c1 + '#&'
        phonecopy = phonecopy.replace(c1, c2)
    #print("In syllabilfy function:", phonecopy)
    phonecopy = rec_replace(phonecopy, "&#&hq&","&hq&#&")
    ...
    #print("In syllabify function before:", phonecopy)
    if g.langId==10:
        temp=phonecopy.split("&")
        # if temp[-2]=="#":
        #     i=-3
        # else:
        #     i=-2
        # #print("print",temp[i],temp[i-1])
        # if temp[i]=="hq":
        #     if temp[i-1] in consonantList:
        #         temp[i]="av&#&hq&av"
        #     else:
        #         temp[i]="#&hq&"+temp[i-1]
        # phonecopy= "&".join(temp)
    
        hard_sibi_asp=['kh','ch','th','ph','sh','sx','s']
        #refer this for more info https://upasanayoga.org/AKAruna/docs/SG.htm#SG.1.5.b
        print(temp)
        for i in range(1,len(temp)):   
            if temp[i]=="hq":
                if i+2<len(temp)and temp[i+1] =="#" and temp[i+2] in hard_sibi_asp:
                    #if i+1<len(temp)and temp[i+2] in hard_sibi_asp:
                    temp[i]=''
                elif i+1<len(temp)and temp[i+1] in hard_sibi_asp:
                    temp[i]=''

                elif temp[i-1] in consonantList:
                    temp[i]="av&#&hq&av&#&"
                    print(temp[i],"two")
                else:
                    temp[i]="#&hq&"+temp[i-1]+"&#&"
        phonecopy= "&".join(temp)
        #print("In syllabify function after:", phonecopy)
...
def SplitSyllables(g : GLOBALS, input : str) -> int:
    ...
    #print("input-b:",incopy,g.syllableList)
    if g.flags.DEBUG:
        for i in range(ln):
...
def WritetoFiles(g : GLOBALS) -> int:
    ...
    # //syllable
        ...
        for i in range(g.syllableCount):
            ...
            j = 0
    #print("g.words.outputText:",g.words.outputText)
    g.words.outputText = g.words.outputText.replace('#', '')
    g.words.outputText = g.words.outputText.replace('  ', ' ')
    if g.flags.DEBUG:
